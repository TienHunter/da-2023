<template>
  <div class="container-login">
    <div class="wrap-login">
      <a-form ref="formRef" name="custom-validation" :model="registerInfo" :rules="rules" autocomplete="off"
        size="large" class="w-full" layout="vertical" @finish="onFinish" @validate="onValidate">
        ...
        <a-form-item name="fullname" :label="$t('auth.Fullname')">
          <a-input v-model:value="registerInfo.fullname" :placeholder="$t('auth.Fullname')" autocomplete="off" />
        </a-form-item>
        <a-form-item name="username" :label="$t('auth.UsernameHint')">
          <a-input v-model:value="registerInfo.username" :placeholder="$t('auth.UsernameHint')" autocomplete="off" />
        </a-form-item>
        <a-form-item name="email" :label="$t('auth.EmailHint')">
          <a-input v-model:value="registerInfo.email" :placeholder="$t('auth.EmailHint')" autocomplete="off" />
        </a-form-item>
        <a-form-item name="password" :label="$t('auth.PasswordHint')">
          <a-input-password v-model:value="registerInfo.password" autocomplete="off" />
        </a-form-item>
        <a-form-item name="confirmPassword" :label="$t('auth.ConfirmPasswordHint')">
          <a-input-password v-model:value="registerInfo.confirmPassword" autocomplete="off" />
        </a-form-item>
        <a-form-item>
          <a-button type="primary" class="w-full" html-type="submit" :loading="isLoading">{{ $t("auth.Register")
            }}</a-button>
        </a-form-item>

        <!-- Đường dẫn "login" -->
        <a-form-item>
          <span class="w-full flex justify-center gap-2">{{ $t("auth.YouHaveAccount") }}
